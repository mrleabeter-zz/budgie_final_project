class FavoritesController < ApplicationController
  ...
  before_action :set_user_favorite
  # GET /favorites
  # GET /favorites.json
  def index
  ...
  def update
    respond_to do |format|
      if @favorite.update(favorite_params)
        format.html { redirect_to user_favorites_path(@user), notice: 'Favorite was successfully updated.' }
  ...
  def destroy
    @favorite.destroy
    respond_to do |format|
      format.html { redirect_to user_favorites_path(@user)}
      format.json { head :no_content }
    end
  end
